...
function loadData(id) {    
        axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/permission/"+id+"/edit",
        responseType: 'json'        
        })
        .then(function (response) {
            ...
            $('#nama').val(name)  
            let existingRoles = []   
            $.each(response.data.data.roles, function(key, item) {              
            ...
            for (var i = 0; i < input_checkbox.length; i++) {   
                if(existingRoles.includes(parseInt(input_checkbox[i].value))) {
...
$(document).ready(function () {   
    ...
    function submitPermissionForm(){
        ...
        let tempRoles = []
        for (var i = 0; i < input_checkbox.length; i++) {   
            console.log(input_checkbox[i]);
            if(input_checkbox[i].checked) {
                tempRoles.push(input_checkbox[i].value)
            }
        } 
       finalRoles = tempRoles.join([separator = ','])
       formData.append('roles', finalRoles)
        formData.append('user_id', {{Auth::user()->id}})
        axios({
        method: 'post',
        url: api_url+"api/permission",
        responseType: 'json',
        data: formData
        })
        .then(function (response) {
...
axios({
        ...
        .then(function (response) {
            ...
            $.each(roles, function(key, item) {
              ...
              let html = `<div class="form-group row col-12" style="margin-top:20px;">
                  ...
                  <div class="col-6">
                  <input class="form-check-input" type="checkbox" id="roleInput" name="" value="`+item.id+`">
                  </div>
                </div>`
                $('#roles').append(html);  
            })
    ...
    function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivatePermission",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivatePermission",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
