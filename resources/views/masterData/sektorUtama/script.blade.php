<script>
      ...
      const BahaginaList ={};
        function changeKategori(){
            d = document.getElementById("bahagian").value;
            var code = BahaginaList[d]
            $('#code_bahagian').val(code)   
            console.log(code)
        ...
        function loadflow(data) {
            ...
            var url = "{{ route('master.sektor',[ ':id']) }}"
            url = url.replace(":id", data)
            console.log(url)
            window.location.href = url
      }
      $('#add_selenggara_data_modal').on('hidden.bs.modal', function () {
                    $('#sektorUtamaForm')[0].reset();
                });

      function loadData(id) {    
        axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/SektorUtama/" + id,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.id
            var code = response.data.data.kod_sektor_utama
            var name = response.data.data.name
            var description =  response.data.data.penerangan_sektor_utama
            var bahagian = response.data.data.bahagian_id
            console.log(id)     
            //$('#modal_body').html(id)
            $('#id').val(id)            
            $('#code').val(code)            
...
$(document).ready(function () {   
    ...
    $("#sektorUtamaForm").submit(function(event){
		submitsektorUtamaForm();
		return false;
	});
    function submitsektorUtamaForm(){
        //console.log(document.lookup.name.value)
        var formEl = document.forms.sektorUtamaForm;
        ...
        axios({
        method: 'post',
        url: api_url+"api/lookup/SektorUtama",
        responseType: 'json',
        data: formData
        })
        .then(function (response) {
    ...
    if({{$bahagianId}} == 0) {
        negeriurl ="{{ env('API_URL') }}"+"api/lookup/BahagianEpu/list"
    } else {
        negeriurl ="{{ env('API_URL') }}"+"api/lookup/BahagianEpu/list?id=" + {{$bahagianId}}
    }
    ...
    axios({
        ...
        .then(function (response) {
            ...
            if(response.data.data.length == 1) {
              kategori = response.data.data[0].name                 
              document.getElementById("headerBreadcrum").textContent="  -  " + kategori;  
              code_bahagian = response.data.data[0].kod_bahagian; 
              $('#code_bahagian').val(code_bahagian);             
              //document.getElementById("code").textContent= response.data.data[0].kod_negeri;              
            }else {
              code_bahagian = response.data.data[0].kod_bahagian; 
              $('#code_bahagian').val(code_bahagian)
            }
            ...
            $.each(response.data.data, function (key, item) {
              BahaginaList[item.id] = item.kod_bahagian
					var opt = item.id;
        ...
        if({{$bahagianId}} == 0) {
            url ="{{ env('API_URL') }}"+"api/lookup/SektorUtama/list"
        } else {
            url ="{{ env('API_URL') }}"+"api/lookup/SektorUtama/list?id=" + {{$bahagianId}}
        }
        axios({
        method: 'get',
        url: url,
        ...
        .then(function (response) {
            ...
            var jps_table =$('#master_data').DataTable({     
              ...
              columnDefs: [
                  {
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              //data=row.value
                              var sektor_count = row.sektor.length
                              data = '<div class="d-flex align-items-center">' +   
                              '<p style="cursor:pointer" onClick="loadflow(\''+row.id+'\')">' + row.name + ' ('+ sektor_count +')</p>' +
                              '</div>'
              ...
              columns: [
                  { data: 'name'},
                  { data: 'kod_sektor_utama'},
                  { data: 'penerangan_sektor_utama'  },
                  { data: 'updated_at'  },          
                  {
                        targets:4,
                        render: function ( data, type, row, meta ) {
    ...
    const api_token = "Bearer "+ window.localStorage.getItem('token')
          function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateSektorUtama",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateSektorUtama",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
