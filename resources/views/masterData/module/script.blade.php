...
function loadflow(data) {
  ...
  function loadData(data) {  
    axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/get_master_details/"+data+"/edit",
        responseType: 'json'        
        })
        .then(function (response) {
            var id = response.data.data.modules.id
            var name = response.data.data.modules.modul_name
            $('#id').val(id)            
            $('#nama').val(name)

            let existingUserTypes = []   
            $.each(response.data.data.userTypes, function(key, item) {              
              existingUserTypes.push(item.user_type)
            })
...
$(document).ready(function () {   
    ...
    function submitroleForm(){        
        ...
        if(tempUserTypes=='')
        {
          document.getElementById("error_module").innerHTML="Sila pilih jenis pengguna"; 
          return false;
        }

        document.getElementById("error_module").innerHTML='';
       ...
       formData.append('userTypes', finalUserTypes)
          ...
          axios({
          method: 'post',
          url: api_url+"api/lookup/add_master_details",
          responseType: 'json',
          data: formData
          })
          .then(function (response) {
  ...
  axios({
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              ...
                              data = '<div class="d-flex align-items-center">' +                                
                                '<p style="cursor:pointer" >' + row.modul_name +'</p>' +
                              '</div>'
                          }
                          return data;
                      }
    ...
    const api_token = "Bearer "+ window.localStorage.getItem('token')
          function activateModul(id){
            Swal.fire({
            text: 'Adakah anda pasti mahu mengaktifkan ini?',
            icon: 'warning',
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/Activatemasterdata",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            Swal.fire({
            text: 'Adakah anda pasti mahu menyahaktifkan ini?',
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#0ACF97',
            cancelButtonColor: '#FA5C7C',
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/Deactivatemasterdata",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
