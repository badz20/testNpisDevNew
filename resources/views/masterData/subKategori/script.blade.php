<script>
      ...
      function loadData(id) {    
        axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/SubKategori/" + id,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.id
            var code = response.data.data.kod_sub_kategori
            var name = response.data.data.name
            var description =  response.data.data.penerangan_sub_kategori
            var kategori = response.data.data.kategori_id
            console.log(id)     
            //$('#modal_body').html(id)
            $('#id').val(id)            
            $('#code').val(code)            
...
$(document).ready(function () {   
    ...
    function submitdaerahForm(){
        ...
        axios({
        method: 'post',
        url: api_url+"api/lookup/SubKategori",
        responseType: 'json',
        data: formData
        })
        .then(function (response) {
    ...
    if({{$kategoriId}} == 0) {
        negeriurl ="{{ env('API_URL') }}"+"api/lookup/Kategori/list"
    } else {
        negeriurl ="{{ env('API_URL') }}"+"api/lookup/Kategori/list?id=" + {{$kategoriId}}
    }
    ...
    axios({
        ...
        .then(function (response) {
            ...
            if(response.data.data.length == 1) {
              ...
              code_kategori = response.data.data[0].kod_kategori; 
            ...
            $.each(response.data.data, function (key, item) {
              KategoriList[item.id] = item.kod_kategori
					var opt = item.id;
        ...
        if({{$kategoriId}} == 0) {
            url ="{{ env('API_URL') }}"+"api/lookup/SubKategori/list"
        } else {
            url ="{{ env('API_URL') }}"+"api/lookup/SubKategori/list?id=" + {{$kategoriId}}
        }
        axios({
        method: 'get',
        url: url,
        ...
        .then(function (response) {
            ...
            var jps_table =$('#master_data').DataTable({     
              ...
              columnDefs: [
                  {
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              ...
                              '<p style="cursor:pointer" onClick="loadflow(\''+row.id+'\')">' + row.name + ' ('+ mukim_count +')</p>' +
                              '</div>'
              ...
              columns: [
                  { data: 'name'},
                  { data: 'kod_sub_kategori'},
                  { data: 'penerangan_sub_kategori'  },
                  { data: 'updated_at'  },          
                  {
                        targets:4,
                        render: function ( data, type, row, meta ) {
    ...
    function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivatesubKategori",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivatesubKategori",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
