<script>
    ...
    function loadData(id) {    
      axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/Deliverable/" + id,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.deliverable.id
            var heading_id = response.data.data.deliverable.heading.id
            var code = response.data.data.deliverable.code
            var name = response.data.data.deliverable.nama
            var description =  response.data.data.deliverable.catatan
            
            document.getElementById('heading').value = heading_id
            var orderDropdown =  document.getElementById("order");
            console.log(orderDropdown);
            $.each(response.data.data.orders, function (key, item) {
					var el = document.createElement("option");
					el.textContent = item;
					el.value = item;
					orderDropdown.appendChild(el);
                })
            document.getElementById('order').value = response.data.data.deliverable.order

            var order_div =  document.getElementById('order_div')
            order_div.classList.remove('d-none');
...
$(document).ready(function () {   
    ...
    $('#add_selenggara_data_modal').on('hidden.bs.modal', function () {
        console.log('close triggered');
                $('#deliverableForm')[0].reset();
                var order_div =  document.getElementById('order_div')
                order_div.classList.add('d-none');
            }); 
    ...
    $("#deliverableForm").submit(function(event){
		submitDeliverableForm();
		return false;
	});
    function submitDeliverableForm(){
        //console.log(document.lookup.name.value)
        var formEl = document.forms.deliverableForm;
        var formData = new FormData(formEl);
        formData.append('user_id', "{{Auth::user()->id}}")
        
        axios({
        method: 'post',
        url: api_url+"api/lookup/Deliverable",
        responseType: 'json',
        data: formData
        })
        .then(function (response) {
    ...
    if({{$headingId}} == 0) {
        headingurl ="{{ env('API_URL') }}"+"api/lookup/DeliverableHeading/list"
    } else {
        headingurl ="{{ env('API_URL') }}"+"api/lookup/DeliverableHeading/list?heading_id=" + {{$headingId}}
    }
    
    var headingDropdown =  document.getElementById("heading");
    axios({
        method: 'get',
        url: headingurl,
        responseType: 'json',        
        ...
        if({{$headingId}} == 0) {
            url ="{{ env('API_URL') }}"+"api/lookup/Deliverable/list"
        } else {
            url ="{{ env('API_URL') }}"+"api/lookup/Deliverable/list?id=" + {{$headingId}}
        }
        axios({
        method: 'get',
        url: url,
        ...
        .then(function (response) {
            ...
            var jps_table =$('#master_data').DataTable({     
              data: response.data.data.deliverable,
    ...
    function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateDeliverable",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateDeliverable",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
