<script>
        ...
        const SDGList ={};
        const SDGidList ={};
        function changeSasaran(){
            d = document.getElementById("sasaran").value;
            document.getElementById('SDG').value = SDGList[d] 
            document.getElementById('sdgid').value = SDGidList[d]
            //$('#id').val(kementerianList[d])           
        }

        function loadData(id) {    
          axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/indikator-single/" + id,
        responseType: 'json'        
        })
        .then(function (response) {
          
          // $("#modalsection").addClass("d-none")
          //console.log(response.data.data.SDG[0].nama_sdg)  
          var id = response.data.data.id
          var code = response.data.data.BIL
          var name = response.data.data.Indikatori
          var description =  "" 
          var SDG = response.data.data.sdg.nama_sdg
          var sasaran = response.data.data.Sasaran_id
          console.log(id)     
          //$('#modal_body').html(id)
          $('#id').val(id) 
          document.getElementById('sdgid').value = response.data.data.sdg.id         
          $('#code').val(code)            
          $('#nama').val(name)            
...
$(document).ready(function () {   
  $("div.spanner").addClass("show");
    ...
    function submitbahagianForm(){
        ...
        var formEl = document.forms.IndikatorForm;
        ...
        axios({
        method: 'post',
        url: api_url+"api/lookup/update-indikator",
        responseType: 'json',
        data: formData
        })
        .then(function (response) {
    ...
    if({{$bahagianId}} == 0) {
        negeriurl ="{{ env('API_URL') }}"+"api/lookup/sasaran/list"
    } else {
        negeriurl ="{{ env('API_URL') }}"+"api/lookup/sasaran/list/" + {{$bahagianId}}
        
    }
    console.log(negeriurl)
    console.log({{$bahagianId}})
    var SasaranDropdown =  document.getElementById("sasaran");
    axios({
        method: 'get',
        url: negeriurl,
        responseType: 'json',        
        ...
        .then(function (response) {
            //console.log(response.data.data[0].Sasaran)            
            //document.getElementById('SDG').value = response.data.data[0].SDG_id;
            //kementerian = response.data.data[0].kementerian.nama_kementerian;
            if(response.data.data.length == 1) {
              jabatan = response.data.data[0].Sasaran;             
            ...
            $.each(response.data.data, function (key, item) {
              //console.log("jjj"+item.sdg.nama_sdg)		
              SDGList[item.id] = item.sdg.nama_sdg	
              SDGidList[item.id] = 	item.sdg.id

              console.log("ID List: "+SDGidList[item.id])
              var el = document.createElement("option");
              el.textContent = item.Sasaran;
              el.value = item.id;
              SasaranDropdown.appendChild(el);
            })
            $('#SDG').val="";
            $('#sdgid').val="";
    ...
    $('#spatial_label').hide();
      ...
      function spatial(){
        ...
        if({{$bahagianId}} == 0) {
            url ="{{ env('API_URL') }}"+"api/lookup/indikator/listall"
        } else {
            url ="{{ env('API_URL') }}"+"api/lookup/indikator/list?id=" + {{$bahagianId}}
        }
        axios({
        method: 'get',
        url: url,
        ...
        .then(function (response) {
            console.log(response.data.data.all_indikators)
            $("div.spanner").removeClass("show");
                $("div.overlay").removeClass("show");
            
            var jps_table =$('#master_data').DataTable({     
              data: response.data.data.all_indikators,              
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              ...
                              data = '<div class="d-flex align-items-center folder">' +                                                                
                                '<p style="cursor:pointer" onClick="loadflow(\''+row.id+'\')">' + row.Indikatori + '</p>' +
                              '</div>'
    ...
    function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateIndikator",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateIndikator",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
