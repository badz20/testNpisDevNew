...
$('#add_selenggara_data_modal').on('hidden.bs.modal', function () {
                    $('#DeliverableHeadingForm')[0].reset();
                }); 

function loadData(id) {    
        axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/DeliverableHeading/" + id,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.id
            var name = response.data.data.nama
...
$(document).ready(function () {   
    ...
    $("#DeliverableHeadingForm").submit(function(event){
		submitDeliverableHeadingForm();
		return false;
	});
    function submitDeliverableHeadingForm(){
        //console.log(document.lookup.name.value)
        var formEl = document.forms.DeliverableHeadingForm;
        var formData = new FormData(formEl);
        formData.append('user_id', "{{ Auth::user()->id}}")

        axios({
        method: 'post',
        url: api_url+"api/lookup/DeliverableHeading",
        responseType: 'json',
        data: formData
        })
        .then(function (response) {
...
axios.defaults.headers.common["Authorization"] = api_token
        axios({
        method: 'get',
        url: api_url+"api/lookup/DeliverableHeading/list",
        responseType: 'json'
        })
        .then(function (response) {
            console.log(response)
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              ...
                              data = '<div class="d-flex align-items-center">' +                                
                                '<p style="cursor:pointer" onClick="loadflow(\''+row.id+'\')">' + row.nama + '</p>' +   
                              '</div>'
    ...
    function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateDeliverableHeading",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateDeliverableHeading",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
