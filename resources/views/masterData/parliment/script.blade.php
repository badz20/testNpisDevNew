<script>
      ...
      function loadData(id) {    
        axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/Parlimen/" + id,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.id
            var code = response.data.data.kod_parlimen
            var name = response.data.data.nama_parlimen
            var description =  response.data.data.penerangan_parlimen 
...
$(document).ready(function () {   
    ...
    function submitdaerahForm(){
        ...
        var kod = formData.get("code")
        var desired = kod.replace(/[^\w\s]/gi, '')
        console.log(desired)
        console.log(desired.length)
        if(desired.length != 4) {
          alert("kod parliment minimum 4 character")
        }else {
          axios({
          method: 'post',
          url: api_url+"api/lookup/Parlimen",
          responseType: 'json',
          data: formData
          })
          .then(function (response) {
    ...
    if({{$negeriId}} == 0) {
        ...
        negeriurl ="{{ env('API_URL') }}"+"api/lookup/negeri/list?negeri_id=" + {{$negeriId}}
    }
    console.log({{$negeriId}})
    ...
    axios({
        ...
        if({{$negeriId}} == 0) {
            url ="{{ env('API_URL') }}"+"api/lookup/parlimen/list_parlimen"
        } else {
            url ="{{ env('API_URL') }}"+"api/lookup/parlimen/list_parlimen?id=" + {{$negeriId}}
        }
        axios({
        method: 'get',
        url: url,
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              ...
                              data = '<div class="d-flex align-items-center">' +                                                                
                                '<p style="cursor:pointer" onClick="loadflow(\''+row.id+'\')">' + row.nama_parlimen + '</p>' +
                              '</div>'
    ...
    const api_token = "Bearer "+ window.localStorage.getItem('token')
          function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateParlimen",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateParlimen",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
