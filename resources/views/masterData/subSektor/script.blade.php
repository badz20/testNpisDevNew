<script>
        ...
        const bahagianList ={};
        const sektorUtamaList ={};
        function changeSektor(){
            d = document.getElementById("sektor").value;
            document.getElementById('sektorUtama').value = sektorUtamaList[d]            
      ...
      $('#add_selenggara_data_modal').on('hidden.bs.modal', function () {
                    $('#SubSektorForm')[0].reset();
                });

      function loadData(id) {    
        axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/SubSektor/" + id,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.id
            var code = response.data.data.kod_sub_sektor
            var name = response.data.data.name
            var description =  response.data.data.penerangan_sub_sektor
            var bahagian = response.data.data.bahagian.name
            var sektorUtama = response.data.data.sektor_utama.name
            var sektor = response.data.data.sektor_id
            console.log(id)     
            //$('#modal_body').html(id)
            $('#id').val(id)            
            $('#code').val(code)            
...
$(document).ready(function () {   
    ...
    $("#SubSektorForm").submit(function(event){
		submitSubSektorForm();
		return false;
	});
    function submitSubSektorForm(){
        //console.log(document.lookup.name.value)
        var formEl = document.forms.SubSektorForm;
        ...
        axios({
        method: 'post',
        url: api_url+"api/lookup/SubSektor",
        responseType: 'json',
        data: formData
        })
        .then(function (response) {
            ...
            if(response.data.code == 200) {
              ...
              $("#tutup").click(function(){
                ...
                if(response.data.error.errorInfo[1] == 1062){
                    alert('duplicate key for kod sub sector')
                }else {
    ...
    if({{$subSektorId}} == 0) {
        negeriurl ="{{ env('API_URL') }}"+"api/lookup/Sektor/list"
    } else {
        negeriurl ="{{ env('API_URL') }}"+"api/lookup/Sektor/list?sektor_id=" + {{$subSektorId}}
    }
    ...
    axios({
        ...
        .then(function (response) {
            ...
            document.getElementById('sektorUtama').value = response.data.data[0].sektor_utama.name;
            var bahagian = response.data.data[0].bahagian.name;
            var sektorUtama = response.data.data[0].sektor_utama.name;
            if(response.data.data.length == 1) {
              sektor = response.data.data[0].name
              document.getElementById("headerBreadcrum").textContent= "- " + bahagian + "/" + sektorUtama + "/" + sektor;
              //$('#headerBreadcrum').val(negeri)
            }
            $.each(response.data.data, function (key, item) {
                //console.log(item)		
                    bahagianList[item.id] = item.bahagian.name
                    sektorUtamaList[item.id] = item.sektor_utama.name
					var el = document.createElement("option");
					el.textContent = item.name;
					el.value = item.id;
					sektorDropdown.appendChild(el);
                })
        });

        
        if({{$subSektorId}} == 0) {
            url ="{{ env('API_URL') }}"+"api/lookup/SubSektor/list"
        } else {
            url ="{{ env('API_URL') }}"+"api/lookup/SubSektor/list?id=" + {{$subSektorId}}
        }
        axios({
        method: 'get',
        url: url,
    ...
    const api_token = "Bearer "+ window.localStorage.getItem('token')
          function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateSubSektor",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateSubSektor",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
