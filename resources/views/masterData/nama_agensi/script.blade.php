...
function loadflow(data) {
  ...
  function loadData(data) {    
    axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/nama_agensi/" + data,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.id
            var name = response.data.data.nama_agensi
            console.log(id)     
            //$('#modal_body').html(id)
            $('#id').val(id)            
            $('#nama').val(name)            
...
$(document).ready(function () {   
    ...
    function submitbahagianEpuForm(){        
        ...
        id = formData.get('id')        
          ...
          axios({
          method: 'post',
          url: api_url+"api/lookup/nama_agensi",
          responseType: 'json',
          data: formData
          })
          .then(function (response) {
...
axios.defaults.headers.common["Authorization"] = api_token
        axios({
        method: 'get',
        url: api_url+"api/lookup/nama_agensi/list",
        responseType: 'json'
        })
        .then(function (response) {
            console.log(response)
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              ...
                              data = '<div class="d-flex align-items-center">' +                                
                                '<p style="cursor:pointer">' + row.nama_agensi +'</p>' +
                              '</div>'
    ...
    function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/Activatenama_agensi",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/Deactivatenama_agensi",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
