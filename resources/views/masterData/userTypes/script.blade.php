...
function loadflow(data) {
  ...
  function loadData(data) {  
    ...
    axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/usertypes/"+data+"/edit",
        responseType: 'json'        
        })
        .then(function (response) {
            ...
            $.each(response.data.data.roles, function(key, item) {              
              existingRoles.push(item.role_id)
            })
            ...
            for (var i = 0; i < input_checkbox.length; i++) {   
                ...
                if(existingRoles.includes(input_checkbox[i].value)) {
...
var roles;
    ...
    function submituserTypeForm(){        
        var formEl = document.forms.userTypeForm;
