...
function loadflow(data) {
  ...
  function loadData(data) {    
    axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/BahagianEpu/" + data,
        responseType: 'json'        
        })
...
$(document).ready(function () {   
    ...
    function submitbahagianEpuForm(){        
        ...
        id = formData.get('id')        
          ...
          axios({
          method: 'post',
          url: api_url+"api/lookup/BahagianEpu",
          responseType: 'json',
          data: formData
          })
          .then(function (response) {
...
axios.defaults.headers.common["Authorization"] = api_token
        axios({
        method: 'get',
        url: api_url+"api/lookup/BahagianEpu/list",
        responseType: 'json'
        })
        .then(function (response) {
            console.log(response)
              ...
              columnDefs: [
                  {
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              //data=row.value
                              var utama_count = row.utama.length
                              data = '<div class="d-flex align-items-center">' +                                
                                '<p style="cursor:pointer" onClick="loadflow(\''+row.id+'\')">' + row.name + ' ('+ utama_count +')</p>' +
                              '</div>'
    ...
    function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateBahagianEpu",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateBahagianEpu",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
