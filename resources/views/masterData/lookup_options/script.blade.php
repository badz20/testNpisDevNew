<script>
      ...
      const KategoriList ={};
        function changeSkop(){
            d = document.getElementById("key").value;
      ...
      function loadData(id) {    
        axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/options/" + id,
        responseType: 'json'        
        })
        .then(function (response) {
            ...
            var description =  response.data.data.catatan
            var key = response.data.data.key
            var order_no = response.data.data.order_by
            console.log(id)     
            //$('#modal_body').html(id)
            $('#id').val(id)            
            $('#code').val(code)            
...
$(document).ready(function () {   
    ...
    function submitdaerahForm(){
        ...
        axios({
        method: 'post',
        url: api_url+"api/lookup/options",
        responseType: 'json',
        data: formData
        })
        .then(function (response) {
        ...
        negeriurl ="{{ env('API_URL') }}"+"api/lookup/options/distint/key"
    
    var KategoriDropdown =  document.getElementById("key");
    axios({
        method: 'get',
        url: negeriurl,
        responseType: 'json',        
        ...
        .then(function (response) {
            ...
            //   document.getElementById("headerBreadcrum").textContent="  -  Options" ;  
            ...
            $.each(response.data.data, function (key, item) {
              KategoriList[item.key] = item.key
					var opt = item.key;
    ...
    $('#spatial_label').hide();
      ...
      function spatial(){
          ...
          $('#nonspatial_label').hide();
            ...
            url ="{{ env('API_URL') }}"+"api/lookup/options/list"
              ...
              columnDefs: [
                  {
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              ...
                              '<p style="cursor:pointer" >' + row.value + '</p>' +
                              '</div>'
    ...
    function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateLookupOptions",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateLookupOptions",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
