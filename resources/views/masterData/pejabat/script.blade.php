...
$(document).ready(function () {  
    ...
    const api_url = "{{ env('API_URL') }}"
        ...
        axios({
        method: 'get',
        url: api_url+"api/lookup/pejabat-projek/list_pejabat_projek",
        responseType: 'json'
        })
            ...
            var jps_table =$('#master_data').DataTable({     
              ...
              columnDefs: [
                  ...
                  },
                  {
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                            if(row.dikemaskini_oleh!=undefined){
                                data=row.user.name
                            }  
                            else{
                                data=''
    ...
    function loadData(id) {    
      ...
      axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/pejabat-projek/list_pejabat_projek/" + id,
        responseType: 'json',
        headers: {
        ...
        .then(function (response) {
            ...
            var nama = response.data.data.pajabat_projek            
            console.log(nama)     
            //$('#modal_body').html(id)
            $('#update_id').val(id)                
            $('#edit_nama_komponen').val(nama)            
  ...
  function activateModul(id){
    Swal.fire({
            text: 'Adakah anda pasti mahu mengaktifkan pejabat projek ini?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                axios({
                method: 'POST',
                url: "{{ env('API_URL') }}"+"api/lookup/pejabat_update_status",
                responseType: 'json',
                data:formData,   
                headers: {
                    "Content-Type": "application/json",
    ...
    function deactivateModul(id){
        Swal.fire({
            text: 'Adakah anda pasti mahu menyahaktifkan pejabat projek ini?',
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#0ACF97',
            cancelButtonColor: '#FA5C7C',
  ...
  $("#edit_komponen_Btn").click(function(){
   var id= $("#update_id").val();
   var nama_komponen= $("#edit_nama_komponen").val();
   if(nama_komponen==''){
      alert('Nama Komponen is are required');
   }
   else{  
    var formData = new FormData();
            formData.append("id",id);
            ...
            axios({
                method: 'POST',
                url: "{{ env('API_URL') }}"+'api/lookup/pejabat_update',
                responseType: 'json',
                data:formData,   
                headers: {
                    "Content-Type": "application/json",
...
$("#addKomponenBtn").click(function(){
   var nama_komponen= $("#add_nama_komponen").val();
   if(nama_komponen==''){
      alert('Nama komponen is are required');
   }
   else{  
    var formData = new FormData();
            ...
            axios({
                method: 'POST',
                url: "{{ env('API_URL') }}"+'api/lookup/pejabat_add',
                responseType: 'json',
                data:formData,   
                headers: {
                    "Content-Type": "application/json",
