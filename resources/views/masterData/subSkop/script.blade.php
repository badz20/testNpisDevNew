<script>
      ...
      const KategoriList ={};
        function changeSkop(){
            d = document.getElementById("skop").value;
            var code = KategoriList[d]
            $('#code_skop').val(code)   
            console.log(code)
      ...
      function loadData(id) {    
        axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/sub_skop/" + id,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.id
            var code = response.data.data.sub_skop_code
            var name = response.data.data.sub_skop_name
            var description =  response.data.data.penerangan_sub_skop
            var skop = response.data.data.skop_id
            console.log(id)     
            //$('#modal_body').html(id)
            $('#id').val(id)            
            $('#code').val(code)            
...
$(document).ready(function () {   
    ...
    function submitdaerahForm(){
        ...
        axios({
        method: 'post',
        url: api_url+"api/lookup/sub_skop",
        responseType: 'json',
        data: formData
        })
        .then(function (response) {
    ...
    if({{$skopId}} == 0) {
        negeriurl ="{{ env('API_URL') }}"+"api/lookup/skop/list"
    } else {
        negeriurl ="{{ env('API_URL') }}"+"api/lookup/skop/list?id=" + {{$skopId}}
    }
    var KategoriDropdown =  document.getElementById("skop");
    axios({
        method: 'get',
        url: negeriurl,
        responseType: 'json',        
        ...
        .then(function (response) {
            ...
            if(response.data.data.length == 1) {
              kategori = response.data.data[0].skop_name                 
              document.getElementById("headerBreadcrum").textContent="  -  " + kategori;  
              code_kategori = response.data.data[0].skop_code; 
              $('#code_kategori').val(code_kategori);             
            ...
            $.each(response.data.data, function (key, item) {
              KategoriList[item.id] = item.skop_code
					var opt = item.id;
					var el = document.createElement("option");
					el.textContent = item.skop_name;
					el.value = opt;
					KategoriDropdown.appendChild(el);
                })
        });
    ...
    $('#spatial_label').hide();
      ...
      function spatial(){
        ...
        if({{$skopId}} == 0) {
            url ="{{ env('API_URL') }}"+"api/lookup/sub_skop/list_sub_skop"
        } else {
            url ="{{ env('API_URL') }}"+"api/lookup/sub_skop/list_sub_skop?id=" + {{$skopId}}
        }
        axios({
        method: 'get',
        url: url,
              ...
              columnDefs: [
                  {
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              ...
                              '<p style="cursor:pointer" >' + row.sub_skop_name + '</p>' +
                              '</div>'
    ...
    function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateSubSkop",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateSubSKop",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
