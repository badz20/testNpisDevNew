<script>
        ...
        const negeriList ={};
        const daerahList ={};
        function changeParlimen(){
        d = document.getElementById("parlimen").value;
        console.log(negeriList)
        console.log(d)
    ...
    function loadData(id) {    
      axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/Dun/" + id,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.id
            var code = response.data.data.kod_dun
            var name = response.data.data.nama_dun
            var description =  response.data.data.penerangan_dun
            var negeri = response.data.data.negeri.nama_negeri            
            var parlimen = response.data.data.parlimen_id
            console.log(id)     
            //$('#modal_body').html(id)
            $('#id').val(id)            
            $('#code').val(code)            
...
$(document).ready(function () {   
    ...
    $("#dunForm").submit(function(event){
		submitdunForm();
		return false;
	});
    function submitdunForm(){
        //console.log(document.lookup.name.value)
        var formEl = document.forms.dunForm;
        var formData = new FormData(formEl);
        formData.append('user_id', {{Auth::user()->id}})
        //console.log(formData)
        console.log($('form#dunForm').serialize())
        console.log('form submitted')

        axios({
        method: 'post',
        url: api_url+"api/lookup/Dun",
        responseType: 'json',
        data: formData
        })
        .then(function (response) {
...
console.log({{$parlimenId}})
    if({{$parlimenId}} == 0) {
        parlimenurl ="{{ env('API_URL') }}"+"api/lookup/parlimen/list_parlimen"
    } else {
        parlimenurl ="{{ env('API_URL') }}"+"api/lookup/parlimen/list_parlimen?parlimen_id=" + {{$parlimenId}}
    }
    console.log({{$parlimenId}})
    var parlimenDropdown =  document.getElementById("parlimen");
    axios({
        method: 'get',
        url: parlimenurl,
        responseType: 'json',        
        ...
        .then(function (response) {
            console.log(response)            
            //document.getElementById('daerah').value = response.data.data[0].daerah.nama_daerah;
            document.getElementById('negeri').value = response.data.data[0].negeri.nama_negeri;
            if(response.data.data.length == 1) {
              parlimen = response.data.data[0].nama_parlimen              
              document.getElementById("headerBreadcrum").textContent="  -  " + parlimen;               
            ...
            $.each(response.data.data, function (key, item) {
                //console.log(item)		
                    ...
                    // daerahList[item.id] = item.daerah.nama_daerah			
        ...
        if({{$parlimenId}} == 0) {
            url ="{{ env('API_URL') }}"+"api/lookup/dun/list_dun"
        } else {
            url ="{{ env('API_URL') }}"+"api/lookup/dun/list_dun?id=" + {{$parlimenId}}
        }
        axios({
        method: 'get',
        url: url,
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              ...
                              data = '<div class="d-flex align-items-center">' +                                
                                '<p style="cursor:pointer">' + row.nama_dun + '</p>'
                              '</div>'
    ...
    function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateDun",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateDun",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
