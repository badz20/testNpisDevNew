...
function loadflow(data) {
      ...
      function loadData(id) {    
        axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/Gred/" + id,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.id
            var code = response.data.data.kod_gred
            var name = response.data.data.nama_gred
            var description =  response.data.data.penerangan_gred
...
$(document).ready(function () {   
    ...
    $("#GredForm").submit(function(event){
		submitGredForm();
		return false;
	});
    function submitGredForm(){
        //console.log(document.lookup.name.value)
        var formEl = document.forms.GredForm;
...
axios.defaults.headers.common["Authorization"] = api_token
        axios({
        method: 'get',
        url: api_url+"api/lookup/gredjawatan/list_gred",
        responseType: 'json'
        })
        .then(function (response) {
            console.log(response)
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              ...
                              data = '<div class="d-flex align-items-center">' +                                
                                '<p style="cursor:pointer" onClick="loadflow(\''+row.id+'\')">' + row.nama_gred + '</p>' +   
                              '</div>'
    ...
    function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateGred",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateGred",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
