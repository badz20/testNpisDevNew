...
function loadflow(data) {
      ...
      function loadData(id) {    
        axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/Jawatan/" + id,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.id
            var code = response.data.data.kod_jawatan
            var name = response.data.data.nama_jawatan
            var description =  response.data.data.penerangan_jawatan
...
$(document).ready(function () {   
    ...
    $("#jawatanForm").submit(function(event){
		submitjawatanForm();
		return false;
	});
    function submitjawatanForm(){
        //console.log(document.lookup.name.value)
        var formEl = document.forms.jawatanForm;
        ...
        axios({
        method: 'post',
        url: api_url+"api/lookup/Jawatan",
        responseType: 'json',
        data: formData
        })
        .then(function (response) {
...
axios.defaults.headers.common["Authorization"] = api_token
        axios({
        method: 'get',
        url: api_url+"api/lookup/jawatan/list_jawatan",
        responseType: 'json'
        })
        .then(function (response) {
            console.log(response)
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              ...
                              data = '<div class="d-flex align-items-center">' +                                
                                '<p style="cursor:pointer" onClick="loadflow(\''+row.id+'\')">' + row.nama_jawatan + '</p>' +   
                              '</div>'
    ...
    function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateJawatan",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateJawatan",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
