...
function loadflow(data) {
            ...
            var url = "{{ route('master.sektor.utama',[ ':id']) }}"
            url = url.replace(":id", data)
            console.log(url)
            window.location.href = url
      }
      $('#add_selenggara_data_modal').on('hidden.bs.modal', function () {
                    $('#bahagianEpuForm')[0].reset();
                }); 
  function loadData(data) {    
    axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/Belanja_Mengurus_Skop/" + data,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.id
            var name = response.data.data.nama_belanga_menguru_skop
            console.log(id)     
            //$('#modal_body').html(id)
            $('#id').val(id)            
            $('#nama').val(name)            
...
$(document).ready(function () {   
    ...
    $("#bahagianEpuForm").submit(function(event){
		submitbahagianEpuForm();
		return false;
	});
    function submitbahagianEpuForm(){        
        var formEl = document.forms.bahagianEpuForm;
        ...
        id = formData.get('id')        
          ...
          axios({
          method: 'post',
          url: api_url+"api/lookup/Belanja_Mengurus_Skop",
          responseType: 'json',
          data: formData
          })
          .then(function (response) {
...
axios.defaults.headers.common["Authorization"] = api_token
        axios({
        method: 'get',
        url: api_url+"api/lookup/Belanja_Mengurus_Skop/list",
        responseType: 'json'
        })
        .then(function (response) {
            console.log(response)
            ...
            var jps_table =$('#master_data').DataTable({     
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              ...
                              data = '<div class="d-flex align-items-center">' +                                
                                '<p style="cursor:pointer">' + row.nama_belanga_menguru_skop +'</p>' +
                              '</div>'
              ...
              columns: [
                  { data: 'nama_belanga_menguru_skop'},         
                  {
                        targets:1,
                        render: function ( data, type, row, meta ) {
    ...
    function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateBelanja_Mengurus_Skop",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateBelanja_Mengurus_Skop",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
