<script>
      ...
      $('#add_selenggara_data_modal').on('hidden.bs.modal', function () {
                    $('#KementerianForm')[0].reset();
                });
function loadflow(data) {
            var base_url = window.location.origin;
            //var url = base_url + '/' + data;
            var url = "{{ route('master.jabatan',[ ':id']) }}"
      ...
      function loadData(id) {    
        axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/Kementerian/" + id,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.id
            var code = response.data.data.kod_kementerian
            var name = response.data.data.nama_kementerian
            var description =  response.data.data.penerangan_kementerian
...
$(document).ready(function () {   
    ...
    $("#KementerianForm").submit(function(event){
		submitKementerianForm();
		return false;
	});
    function submitKementerianForm(){
        //console.log(document.lookup.name.value)
        var formEl = document.forms.KementerianForm;
        ...
        axios({
        method: 'post',
        url: api_url+"api/lookup/Kementerian",
        responseType: 'json',
        data: formData
        })
        .then(function (response) {
...
axios.defaults.headers.common["Authorization"] = api_token
        axios({
        method: 'get',
        url: api_url+"api/lookup/kementerian/list",
        responseType: 'json'
        })
        .then(function (response) {
            console.log(response)
            ...
            var jps_table =$('#master_data').DataTable({     
              ...
              columnDefs: [
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              ...
                              data = '<div class="d-flex align-items-center">' +                               
                                '<p style="cursor:pointer" onClick="loadflow(\''+row.id+'\')">' + row.nama_kementerian + '</p>' +   
                              '</div>'
                  ...
                  },
                  {
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                                  data=row.penerangan_kementerian
                          }
                          return data;
                      }
                  },                  
    ...
    const api_token = "Bearer "+ window.localStorage.getItem('token')
          function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateKementerian",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateKementerian",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
