...
function loadflow(data) {
  ...
  function loadData(data) {    
    axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/Negeri/" + data,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.id
            var code = response.data.data.kod_negeri
            var name = response.data.data.nama_negeri
            var description =  response.data.data.penerangan_negeri   
            console.log(id)     
            //$('#modal_body').html(id)
            $('#id').val(id)            
            $('#code').val(code)            
...
axios.defaults.headers.common["Authorization"] = api_token
        axios({
        method: 'get',
        url: api_url+"api/lookup/negeri/list_negeri",
        responseType: 'json'
        })
        .then(function (response) {
            console.log(response)
              ...
              columnDefs: [
                  {
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              //data=row.value
                              var daerah_count = row.daerah.length
                              data = '<div class="d-flex align-items-center">' +                                
                                '<p style="cursor:pointer" onClick="loadflow(\''+row.id+'\')">' + row.nama_negeri + ' ('+ daerah_count +')</p>' +
                              '</div>'
    ...
    const api_token = "Bearer "+ window.localStorage.getItem('token')
          function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateNegeri",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateNegeri",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
