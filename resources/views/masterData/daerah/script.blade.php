    ...
    function spatial(){
      ...
      const NegeriList ={};
        function changeNegeri(){
            ...
            var code = NegeriList[d]
            $('#code_negeri').val(code)   
            console.log(code)
      ...
      function loadData(id) {    
        axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/Daerah/" + id,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.id
            var code = response.data.data.kod_daerah
            var name = response.data.data.nama_daerah
            var description =  response.data.data.penerangan_daerah 
            var negeri = response.data.data.negeri_id
            console.log(id)     
            //$('#modal_body').html(id)
            $('#id').val(id)            
            $('#code').val(code) 
            $('#code_negeri').val(response.data.data.kod_negeri)            
            $('#nama').val(name)            
...
$(document).ready(function () {   
    ...
    if({{$negeriId}} == 0) {
        negeriurl ="{{ env('API_URL') }}"+"api/lookup/negeri/list"
    } else {
        negeriurl ="{{ env('API_URL') }}"+"api/lookup/negeri/list?id=" + {{$negeriId}}
    }
    ...
    axios({
        ...
        .then(function (response) {
            ...
            if(response.data.data.length == 1) {
              negeri = response.data.data[0].nama_negeri                  
              document.getElementById("headerBreadcrum").textContent="  -  " + negeri;  
              code_negeri = response.data.data[0].kod_negeri; 
              $('#code_negeri').val(code_negeri);             
        ...
        if({{$negeriId}} == 0) {
            url ="{{ env('API_URL') }}"+"api/lookup/daerah/list_daerah"
        } else {
            url ="{{ env('API_URL') }}"+"api/lookup/daerah/list_daerah?id=" + {{$negeriId}}
        }
        axios({
        method: 'get',
        url: url,
              ...
              columnDefs: [
                  {
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              //data=row.value
                              var mukim_count = row.mukim.length
                              data = '<div class="d-flex align-items-center">' +   
                              '<p style="cursor:pointer" onClick="loadflow(\''+row.id+'\')">' + row.nama_daerah + ' ('+ mukim_count +')</p>' +
                              '</div>'
                  ...
                  },
                  {
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                                  data=  row.kod_negeri + row.kod_daerah
                          }
                          return data;
                      }
                  },     
    ...
    function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateDaerah",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateDaerah",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
