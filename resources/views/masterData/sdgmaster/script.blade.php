...
function loadflow(data) {
            ...
            var url = "{{ route('master.sasaran',[ ':id']) }}"
            console.log(url)
      ...
      function loadData(id) {    
        axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/sdg-details/" + id,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response.data.data[0].id)  
            var id = response.data.data[0].id
            var code = response.data.data[0].kod_sdg
            var name = response.data.data[0].nama_sdg
            var description =  ""
...
$(document).ready(function () {  
    ...
    $("#SDGForm").submit(function(event){
		submitSDGForm();
		return false;
	});
    function submitSDGForm(){
        //console.log(document.lookup.name.value)
        var formEl = document.forms.SDGForm;
        ...
        axios({
        method: 'post',
        url: api_url+"api/lookup/update-SDG",
        responseType: 'json',
        data: formData
        })
        .then(function (response) {
...
axios.defaults.headers.common["Authorization"] = api_token
        axios({
        method: 'get',
        url: api_url+"api/lookup/sdg/list",
        responseType: 'json'
        })
        .then(function (response) {
            console.log(response)
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              ...
                              data = '<div class="d-flex align-items-center">' +                               
                                '<p style="cursor:pointer" onClick="loadflow(\''+row.id+'\')">' + row.nama_sdg + '</p>' +   
                              '</div>'
    ...
    const api_token = "Bearer "+ window.localStorage.getItem('token')
          function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateSDG",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateSDG",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
