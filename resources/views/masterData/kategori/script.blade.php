...
function loadflow(data) {
            ...
            var url = "{{ route('master.sub.kategori',[ ':id']) }}"
  ...
  function loadData(data) {    
    axios({
        method: 'get',
        url: "{{ env('API_URL') }}"+"api/lookup/Kategori/" + data,
        responseType: 'json'        
        })
        .then(function (response) {
            console.log(response)  
            var id = response.data.data.id
            var code = response.data.data.kod_kategori
            var name = response.data.data.name
            var description =  response.data.data.penerangan_kategori  
            console.log(id)     
            //$('#modal_body').html(id)
            $('#id').val(id)            
            $('#code').val(code)            
...
$(document).ready(function () {   
    ...
    function submitkategoriForm(){        
        var formEl = document.forms.kategoriForm;
        ...
        id = formData.get('id')        
          ...
          axios({
          method: 'post',
          url: api_url+"api/lookup/Kategori",
          responseType: 'json',
          data: formData
          })
          .then(function (response) {
...
axios.defaults.headers.common["Authorization"] = api_token
        axios({
        method: 'get',
        url: api_url+"api/lookup/Kategori/list",
        responseType: 'json'
        })
        .then(function (response) {
            console.log(response)
              ...
              columnDefs: [
                  {
                      ...
                      render: function ( data, type, row, meta ) {
                          if(type === 'display'){
                              //data=row.value
                              var subJenis_count = row.sub_jenis.length
                              data = '<div class="d-flex align-items-center">' +                                
                                '<p style="cursor:pointer" onClick="loadflow(\''+row.id+'\')">' + row.name + ' ('+ subJenis_count +')</p>' +
                              '</div>'
    ...
    function activateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/ActivateKategori",
            responseType: 'json',
            data:formData,   
            headers: {
          ...
          function deactivateModul(id){
            ...
            axios({
            method: 'POST',
            url: "{{ env('API_URL') }}"+"api/lookup/DeactivateKategori",
            responseType: 'json',
            data:formData,   
            headers: {
                "Content-Type": "application/json",
