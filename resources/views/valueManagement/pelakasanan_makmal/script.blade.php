...
<link rel="stylesheet" type="text/css" href="https://cdn.jsdelivr.net/npm/daterangepicker/daterangepicker.css" />
      ...
      $(document).ready(function () {
        ...
        document.getElementById("va_tandatanganLink").style.opacity = "0.5";
        ...
        localStorage.setItem("loader_count", 7);

        $("#card1").addClass("d-none")
        $("#inlineCheckbox1").click(function(){

          $("#KemaskiniKepadaBahagianBtn").prop("disabled", true);
          document.getElementById("KemaskiniKepadaBahagianBtn").style.opacity = "0.5";
          $("#va_tandatanganLink").prop("disabled", false);
          document.getElementById("va_tandatanganLink").style.opacity = "1";
        ...
        $('#inlineCheckbox1').click(function(){
            if($(this).is(":checked")){
              ...
              document.getElementById("KemaskiniKepadaBahagianBtn").style.opacity = "1";
              $("#va_tandatanganLink").prop("disabled", true);
              document.getElementById("va_tandatanganLink").style.opacity = "0.5";
            }
        });
        ...
        var isVEclicked= localStorage.getItem('isVEclicked');
        if(isVEclicked=='clicked'){
          const api_token = "Bearer "+ window.localStorage.getItem('token')
          axios.defaults.headers.common["Authorization"] = api_token
          axios({
          method: 'get',
          url: "{{ env('API_URL') }}"+"api/project/maklumat_pelakasanaan?id="+{{$kod_projek}},
          responseType: 'json',            
        })
        .then(function (response) { 
            ...
            if(response.data.data.length>0)
            {
              ...
              document.getElementById("SignedDocSaveBtn").style.display = true; 
              document.getElementById("SignedDocSaveBtn").style.opacity = "0.5";
              $('#SignedDocSaveBtn').addClass('d-none');
              document.getElementById("tarikh_terima").disabled = false; 
              document.getElementById("terima_file_name").disabled = false; 
              document.getElementById("simp_tandatangan").disabled = false; 
              $('#simp_tandatangan').removeClass('d-none');
            }
            else
            {
              document.getElementById("inlineCheckbox1").disabled= true;
              $('#SignedDocSaveBtn').removeClass('d-none');
              document.getElementById("SignedDocSaveBtn").disabled = false; 
              document.getElementById("SignedDocSaveBtn").style.opacity = "1";
              document.getElementById("tarikh_terima").disabled = true; 
              document.getElementById("terima_file_name").disabled = true; 
              document.getElementById("simp_tandatangan").disabled = true; 
              $('#simp_tandatangan').addClass('d-none');
            }
            
            for(i=0;i<response.data.data.length;i++){
                  var kemuka_file_name=tabledata[i].kemuka_file_name;// alert(kemuka_file_name);

                  if(kemuka_file_name)
                  { //alert('hh')
                        $('#kemuka_file_name').attr('value',kemuka_file_name);
                        $("#filePreview2").attr('src','{{ asset('assets/pdf.jpg.png') }}');
                        $("#fileName2").text(kemuka_file_name);
                        $("#Uploadfile2").addClass('d-none')
                        $("#removefile2").addClass('d-none')
                        $("#fileUploaded2").removeClass('d-none')
                  }

                  document.getElementById("tarikh_kemuka").value = tabledata[i].tarikh_kemuka; 
                  document.getElementById("tarikh_kemuka").disabled=true;

                  var id=tabledata[i].id;
                  if(tabledata[i].terima_file_name!=null)
                  {
                    var terima_file_name=tabledata[i].terima_file_name;
                          $('#terima_file_name').attr('value',terima_file_name);
                          $("#filePreview1").attr('src','{{ asset('assets/pdf.jpg.png') }}');
                          $("#fileName1").text(terima_file_name);
                          $("#Uploadfile1").addClass('d-none')
                          $("#removefile1").addClass('d-none')
                          $("#fileUploaded1").removeClass('d-none') 

                          document.getElementById("simp_tandatangan").disabled = true; 
                          document.getElementById("simp_tandatangan").style.opacity = "0.5";
                  }
                  else{
                  var terima_file_name='Tiada Maklumat'
                  }

                if(tabledata[i].tarikh_terima!=null){
                  var tarikh_terima=tabledata[i].tarikh_terima;
                  document.getElementById("tarikh_terima").value = tabledata[i].tarikh_terima; 
                }
                else{
                  var tarikh_terima='dd/mm/yyyy'
                }
              ...
              '<tr>'+
                ...
                '</td>'+
                '<td>'+
                  tabledata[i].tarikh_kemuka+
                '</td>'+
                '<td>'+
                  '<a onclick="kemukafileDownload(\''+id+'\',\''+kemuka_file_name+'\')" id="kemuka_file'+tabledata[i].id+'" style="text-decoration:none;color:black;cursor:pointer">'+tabledata[i].kemuka_file_name+'</a>'+
                '</td>'+
                '<td>'+
                  tarikh_terima+
                '</td>'+
                '<td>'+
                  '<a onclick="terimafileDownload(\''+id+'\',\''+terima_file_name+'\')" id="kemuka_file'+tabledata[i].id+'" style="text-decoration:none;color:black;cursor:pointer">'+terima_file_name+'</a>'+
                '</td>'+
              '</tr>')
            }
          ...
          document.getElementById('va_tandatanganLink').innerHTML = "Hantar untuk Tandatangan  Laporan";
        }
        else{
          $("#card1").addClass("d-none")
          $("#card2").removeClass("d-none")
          document.getElementById('va_tandatanganLink').innerHTML = "Hantar ke Penyelaras";
        }
          ...
          if(output.length > 0) {
                  $.each(output, function (key, item) {
                    ...
                    $('#tbodyoldoutput').append(`<tr >
                          ...
                          <td>
                            <textarea disabled style="font-size:0.8rem;" class="form-control text-center" type="text" value="`+item.output_proj+`">`+item.output_proj+`</textarea>
                              </td>
                          <td>
                            <input disabled style="font-size:0.8rem;" class="form-control text-center" type="text" value="`+item.Kuantiti+`">
                              </td>
                          <td>
      ...
      .then(function (response) { 
              ...
              if(project.status_perlaksanaan == '27') {
                ...
                if(user_bahagian != 'BKOR') {
                  ...
                  // document.getElementById('penyeleras').style.display = 'block';
                // document.getElementById('penyeleras_buttons').style.display = 'block';
           ...
           var length_obj= window.localStorage.getItem('obj_length');
            ...
            readonly = 'readonly'
              ...
              if(objektifData.length > 0) {
                    $.each(objektifData, function (key, item) {
                      ...
                      button = ''
                      if(current_status == '26' || current_status == '28') {
              ...
              if(skopData.length > 0) {
                    $.each(skopData, function (key, item) {
                      ...
                      <input class="form-control text-center" type="hidden" value="`+item.id+`" name="id" `+readonly+`>
                              <td>
                                <textarea style="font-size:0.8rem;" class="form-control text-center" type="text" `+readonly+` wrap="virtual" value="`+item.skop_selepas+`">`+item.skop_selepas+`</textarea>
                              </td>
                              <td>
                ...
                if(outcomeData.length > 0) {
                    $.each(outcomeData, function (key, item) {
                      ...
                      <input class="form-control text-center" type="hidden" value="`+item.id+`" name="id">
                              <td>
                                <textarea  style="font-size:0.8rem;" style="font-size:0.8rem;" class="form-control text-center" type="text" `+readonly+` value="`+item.outcome_selepas+`">`+item.outcome_selepas+`</textarea>
                              </td>
                              <td>
                                <input  style="font-size:0.8rem;" style="font-size:0.8rem;" class="form-control text-center" type="text" `+readonly+` value="`+item.quantity_selepas+`">
                              </td>
                              <td>
                  ...
                  if(oldoutcomelength.length == 0) {
                    newoutcomelength = newoutcomelength + 1
                          ...
                          $('#tbodyoldoutcome').append(`<tr >
                            ...
                            <td>
                              <textarea  style="font-size:0.8rem;" style="font-size:0.8rem;" disabled class="form-control text-center" type="text" value=""></textarea>
                                </td>
                            <td>
                              <input  style="font-size:0.8rem;" style="font-size:0.8rem;" disabled class="form-control text-center" type="text" value="">
                                </td>
                            <td>
                              <input  style="font-size:0.8rem;" style="font-size:0.8rem;" disabled class="form-control text-center" type="text" value="">
                            </td>
            ...
            localStorage.setItem("loader_count", length-1);
                  ...
                  $('#addBtnObjektif').on('click', function () {
                    var length_obj= window.localStorage.getItem('obj_length'); console.log(length_obj)

                    console.log(newobjektiflength)
      ...
      function removeSkopRow(id){
          ...
          {
            newskoplength = newskoplength - 1; console.log(newskoplength)
          }
          calculateNewKos()
      }
    ...
    $('#submit_lampiran').click(function(){
        if(current_status == '28') {
          submitUlasanUser('29','submit')
          submitLampiran('29','submit')
        }else {
          submitLampiran('27','submit')
        }
    ...
    $('input.kos_selepas_makmal').keyup(function(event) {
      ...
      function number_format($num)
      {
        ...
        function submitDataWithout(current_status){
          var status=parseInt(current_status)+parseInt(1);
                const api_url = "{{env('API_URL')}}";  
                          api = api_url+"api/project/update_status_perlaksanaan";
                          const api_token = "Bearer "+ window.localStorage.getItem('token')
                          data={'update_status_perlaksanaan':status+parseInt(1),'kod':{{$kod_projek}}};
                      axios({
                              method: 'post',
                              url: api,
                              responseType: 'json',
    ...
    $("#SignedDocSaveBtn").click(function(){
      ...
      //   return false;
      // }
        if(tarikh_kemuka!=''&&kemuka_file_name!=''){
          $("#tarikh_kemuka_error").text('')
        // $("#tarikh_terima_error").text('')
          $("#kemuka_file_name_error").text('')
    ...
    $("#simp_tandatangan").click(function(){
      var tarikh_terima=$("#tarikh_terima").val()
      var terima_file_name=$("#terima_file_name").val()
      ...
      if(terima_file_name==''){
        ...
        if(tarikh_terima!=''&&terima_file_name!=''){
          $("#tarikh_terima_error").text('')
          $("#terima_file_name_error").text('')
          var allowedExtensions = /(\.pdf|\.png)$/i;
    ...
    $("#va_tandatanganLink").click(function(){
      ...
      const api_url = "{{env('API_URL')}}";  
          api = api_url+"api/project/update_status_perlaksanaan";
          const api_token = "Bearer "+ window.localStorage.getItem('token')
          data={'update_status_perlaksanaan':30,'kod':{{$kod_projek}}};
      axios({
            method: 'post',
            url: api,
            responseType: 'json',
    ...
    $(document).ready(function(){
        const api_token = "Bearer "+ window.localStorage.getItem('token')
          ...
          axios({
          method: 'get',
          url: "{{ env('API_URL') }}"+"api/project/get_pengeculian_data?type=VA&id="+{{$kod_projek}},
          responseType: 'json',            
      })
      .then(function (response) { 
            tabledata=response.data.data
    ...
    function formatdate(date){
      ...
      function previewfile(id,filename){
        const api_url = "{{env('API_URL')}}";  
                      ...
                      console.log(filename)
                      axios({
                        url: update_user_api + '?id=' + id+ '&type=VA',
                        method: 'GET',
                        headers: { "Authorization": api_token, },
                        responseType: 'blob', // important
                      }).then((response) => {
